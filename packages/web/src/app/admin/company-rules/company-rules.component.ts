...
import {IDaysOffScheduler, IDaysOffValue, IDaysOffValueForm} from '../../interfaces/daysOff';
...
@Component({
    ...
    imports: [
        CommonModule, MatInputModule, MatIconModule, MatButtonModule, DaysOffFormComponent, MatCheckboxModule, ReactiveFormsModule, TranslateModule
    ],
    templateUrl: './company-rules.component.html',
    styleUrls: ['./company-rules.component.scss']
})
export class CompanyRulesComponent extends UnsubscribeOnDestroyAdapter implements OnInit  {
  private dataService = inject(DataService);
  private formBuilder = inject(FormBuilder);
  private toastService = inject(ToastrService);
  public companyDaysOff$: Observable<IDaysOffValue>;
  public defaultDaysOff$ = new BehaviorSubject<IDaysOffValue>({} as IDaysOffValue);
  public timeCoefficientScheduler$ = new BehaviorSubject<IDaysOffValue>({} as IDaysOffValue);
  ...
  ngOnInit(): void {
      this.createForm();
      this.companyDaysOff$ = this.dataService.getObservableData('/company-days-off-rules').pipe(
          tap((data: IDaysOffValue) => {
              this.form.patchValue(data);
              this.companyRulesData = data;
              this.daysOffSchedulerData = data.company.daysOffSchedulers;
              this.daysOffSchedulerRules = this.makeDaysOffSchedulerRules(this.daysOffSchedulerData);
          })
      );
  }
    saveDaysOff(): void {
        ...
        company.daysOffSchedulers = this.setDaysOffSchedulerRules(this.daysOffSchedulerData);
        const saved: Subscription = this.dataService.updateData('/company/', company.id,  company)
            .subscribe(
                (r) => {
    ...
    setDaysOffSchedulerRules(data: IDaysOffScheduler[]): IDaysOffScheduler[]{
        const timeCoefficientScheduler: IDaysOffValue = this.timeCoefficientScheduler$.getValue();
