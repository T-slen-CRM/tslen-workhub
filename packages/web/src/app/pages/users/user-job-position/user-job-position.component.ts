...
export class UserJobPositionComponent extends UnsubscribeOnDestroyAdapter{
    constructor(public translateService: LanguageService) { super();}
    private dataService = inject(DataService)
    public titleControl: FormControl = new FormControl('', Validators.required);
    public currentRows: IJobPosition[] = [];
    public columnDefs: ColDef[] = [];
    private destroy$ = new Subject<void>();
    public rowData$: Observable<IJobPosition[]> = this.dataService.getObservableData('/job-position')
        .pipe(tap((r: IJobPosition[]) => {
            this.currentRows = r;
        }));
    ...
    onSubmit(){
        const save: Subscription = this.dataService.postData('/job-position', {title: this.titleControl.value})
    ...
    loadTranslations(): void {
      this.translateService.get([
        'people.job_position.data.id',
        'people.job_position.data.title'
      ]).subscribe(translations => {
        this.columnDefs = [
          { field: 'id', headerName: translations['people.job_position.data.id'], width: 657},
          { field: 'title', headerName: translations['people.job_position.data.title'],width: 657 }
        ];
      });
    }
}
