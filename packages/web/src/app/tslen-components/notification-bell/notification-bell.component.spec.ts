...
describe('NotificationBellComponent', () => {
  ...
  describe('shared state across mounted bell instances', () => {
    ...
    it('still delivers live notifications to a newly mounted instance after the first-mounted instance is destroyed', () => {
      // Regression: live-notification processing used to be claimed permanently by
      // whichever bell instance mounted first (NotificationService.liveNotificationsSubscribed),
      // with no way to hand it off. If that instance was ever destroyed (e.g. across a
      // logout/login cycle), no bell instance could process live arrivals again until a
      // full page reload. The service now owns the socket subscription itself, for its
      // own lifetime, independent of any component's mount/unmount.
      const fixture1 = TestBed.createComponent(NotificationBellComponent);
      fixture1.componentRef.setInput('userId', 7);
      fixture1.detectChanges();
