...
export class UserProfileComponent implements OnInit, OnDestroy {
  ...
  message: string;
  defaultUserAvatar = '';
  headerRoutes = [{value: 'Update profile', url: '', type: 'last' , params: {}}];
  ...
  public googleCalendarData$ = new BehaviorSubject<IGoogleCalendarData>({} as IGoogleCalendarData);
  ...
  constructor(
      private fb: FormBuilder, private dataService: DataService, public dialog: MatDialog,
      private toastr: ToastrService, private authService: AuthenticationService, private router: Router, public translateService: LanguageService
  ) {
  ...
  loadTranslations(): void {
      this.translateService.get(['people.profile.name','people.profile.update_profile']).subscribe((translatedValue: string) => {
        this.mainHeaderPage = translatedValue['people.profile.name']
        this.headerRoutes[0].value = translatedValue['people.profile.update_profile']
    });
  }
  ...
  changePassword(){
    ...
    if (this.changePasswordForm.invalid) {
      ...
      this.form.addControl('password', new FormControl(this.changePasswordForm.get('password').value));
      this.updateUser();
    }
  }
  openUploadDialog(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.fileName){
        const fileUrl = environment.ftpDomain + this.authData.companyId + '-' + this.authData.userId  + '-' + result.fileName;
    ...
    const getUser: Subscription = forkJoin([
      ...
      this.dataService.getObservableData('/job-position')
        ...
        .subscribe(response => {
          ...
          this.form.get('userProbation').get('isProbation').valueChanges.subscribe((value) => {
            if (value === false){
              // reset probation
  ...
  createDefaultFormGroup(){
    ...
    this.form.addControl('userProbation', new FormGroup({
        id: new FormControl(),
    ...
    if (!this.routerId){
      ...
      this.form.addControl('password', new FormControl(this.randomPassword, [Validators.required]));
    }
  ...
  saveUser(){
    ...
    this.setUserGroups();
    this.setChiefId();
    const save: Subscription = this.dataService.postData('/users',
        {...this.form.value}
    ).subscribe((response) => {
      this.loading = false;
  ...
  updateUser(){
    ...
    this.setChiefId();
    this.setDaysOff();
    const update: Subscription = this.dataService.updateData('/users/', this.routerId, {...this.form.value})
        .subscribe((response) => {
      this.loading = false;
      this.userChanges.emit(true);
      this.toastr.success('User`s settings has been saved', 'Saved');
  ...
  setUserGroups(){
    if (this.form.value.userGroups){
      ...
      if (this.form.value.userRelationToGroups && this.form.value.userRelationToGroups.length > 0){
  ...
  setChiefId(){
    if (this.form.value.chiefId){
      ...
      if (this.form.value.userChiefRelations && this.form.value.userChiefRelations.length > 0){
        ...
        userChiefRelations = [{userId: this.userId, chiefId: this.form.value.chiefId}];
      }
