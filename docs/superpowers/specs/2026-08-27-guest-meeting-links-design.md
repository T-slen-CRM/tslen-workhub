...
|---|---|---|
| `id` | uuid, PK | |
| `token` | varchar(64), unique | Stored hashed (one-way), same pattern as `ApiToken` (`hashApiToken` in `src/resources/api-tokens/utils/hash-token.ts`, reused here). This is the deterministic lookup key `MeetingGuestGuard`/`validateToken` query by - never decrypted or redisplayed. |
| `encryptedToken` | varchar(255) | The same plaintext token, separately encrypted (AES-256-GCM, `token-cipher.ts`, key from `MEETING_LINK_TOKEN_KEY`) so `GET /meeting-links` can decrypt and redisplay a link the host created earlier. Kept as its own column rather than replacing `token` because encryption is non-deterministic (random IV per row) and can't serve as a `WHERE` lookup key the way a hash can. |
| `hostUserId` | FK → `Users` | |
...
- `POST /api/meeting-links` (authenticated, global `AuthGuard`) — host
  ...
  token + `roomName`, stores the hashed token plus its encrypted form
  (see entity table above), returns `{ id, token, roomName, expiresAt }`.
  Following the existing `currentRoomLink` pattern in `call.component.ts`
  (built client-side from `window.location.origin`, not
  server-constructed), the frontend builds the shareable URL as
...
- `GET /api/meeting-links` (authenticated) — list the host's own links,
  each including the plaintext `token` (decrypted from `encryptedToken`)
  so the host can copy a link they created earlier, not just the one they
  just created. Never includes the stored hash or the eager `host`
  relation.
