...
import {IPermissionTableColumn, IProjectPermission} from "../../interfaces/taskProjectPermission";
...
import {
  ProjectMemberActionButton
} from '../mat-table-dynamic/data-grid/project-member-action-button/project-member-action-button';
...
export class TaskProjectMembersComponent implements OnInit, OnDestroy {
  ...
  constructor(public dialog: MatDialog,
    ...
    this.tableColumns = [
      ...
      {field: 'userName', headerName: 'User name', permission: ['admin', 'write']},
      {field: 'permission', headerName: 'Permission', permission: ['admin', 'write']},
      // {field: 'created', headerName: 'Created', component: DateTransformComponent, permission: ['admin', 'manager', 'advert']},
      {field: 'delete', headerName: 'Delete', component: ProjectMemberActionButton, permission: ['admin']},
    ];
  ...
  ngOnInit() {
    ...
    if (this.data && this.data.taskProjectPermissions) {
      ...
      this.projectPermissions = this.data.taskProjectPermissions
          .map(item => {
            // for current user
      ...
      this.tableColumns = this.tableColumns.filter(item => item.permission.includes(this.userRoleInProject));
    } else {
        ...
        .subscribe((changedMembersRow: IPermissionChangedRow) => {
          if (changedMembersRow){
            ...
            if (action === 'delete' && !Array.isArray(row)){
              ...
              if ( row && row.id){
                this.projectPermissions = this.projectPermissions.filter(perm => perm.id !== row.id);
              } else if (row.unsavedId){
                this.projectPermissions = this.projectPermissions.filter(perm => perm.unsavedId !== row.unsavedId);
              }
  ...
  getUsers(): void {
    this.preparedUsersData$ = this.dataService.getAllUsers()
        .pipe(map((response: HttpResponse<any>) => {
          const body = response.body;
          this.userData = body;
          return body.map(item => {
            const oneUser = {group: `#${item.id} | ${item.firstName} ${item.lastName} | ${item.email}`,
              value: item.id};
            if (this.usersInPermissionTable.find(user => user.userId === item.id)){
            ...
            tap((data) => {
                this.preparedUsersData = data.filter(item => {
                    return !this.autoSelectedUsers.find(user => user.value === item.value);
                });
            }),
        );
  }
  ...
  //
  onSubmit() {
    this.data.taskProjectPermissions = this.data.taskProjectPermissions.concat(this.selectedUserPermission);
