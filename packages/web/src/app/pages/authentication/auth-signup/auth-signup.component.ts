...
export class AuthSignupComponent implements OnInit, OnDestroy {
  ...
  async onSubmit(){
    this.message = null;
      this.loading = true;
      const createCompany: Subscription = this.dataService.postData('/company', this.companyForm.value)
          .subscribe(
              {
  ...
  sendUserForm(){
      this.loading = true;
      const saveUser: Subscription = this.dataService.postData('/users/signup', {...this.userForm.value})
        .subscribe(
            {
              next: (response: { body: any; status: number; }) => {
                this.loading = false;
