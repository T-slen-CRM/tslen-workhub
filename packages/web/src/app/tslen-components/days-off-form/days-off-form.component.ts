...
export class DaysOffFormComponent extends UnsubscribeOnDestroyAdapter {
  ...
  @Input() public set dataForForm(data: IDaysOffValue){
    ...
    if (data) {
      ...
      // save changes
      const changes: Subscription = this.form.valueChanges.subscribe((value: IDaysOffValue) => {
  ...
  createForm(): void {
    const controls: IDaysOffValue = this.rulesTypes.reduce((acc: IDaysOffValue, rule: IDaysOffSettings) => {
