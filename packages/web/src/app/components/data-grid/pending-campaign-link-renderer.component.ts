...
export class PendingCampaignLinkRendererComponent {
    constructor(
        ...
        if (this.currentUserId !== this.userId){
            ...
            this.dataService.changeUser({data: {userId: this.userId}}).subscribe(response => {
                if (response.status === 200){
                    ...
                    this.loading = false;
                    //[routerLink]="['/campaigns/creatives-list/'+campaignId, {campaignName: campaignName}]
                    this.router.navigate(['/campaigns/creatives-list/'+this.campaignId, {campaignName: this.campaignName}])
                } else {
