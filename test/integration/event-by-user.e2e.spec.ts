...
describe('EventByUserController (e2e)', () => {
    ...
    beforeEach(async () => {
        const moduleFixture: TestingModule = await Test
            .createTestingModule({
                ...
                providers: [
                    {
                        provide: EventsByUserService,
                        useValue: {
                            getEventsByMonth: jest.fn(() => [mockedEventByUser]),
                            getAbsentToday: jest.fn(() => [mockedEventByUser]),
                            create: jest.fn(() => mockedEventByUser),
                            update: jest.fn(() => mockedEventByUser),
                            delete: jest.fn(() => mockedEventByUser),
                            getPending: jest.fn(() => [mockedEventByUser]),
                            approveDisapproveEvent: jest.fn(() => ({ status: 'ok' })),
                        },
                    },
                    {
    ...
    it('events-by-user/pending (GET)', async () => {
        await request(app.getHttpServer())
            .get('/events-by-user/pending')
            .expect(200)
            .expect(({ body }) => {
                expect(body).toEqual([mockedEventByUser]);
            });
    });
    it('should call /events-by-user/approve:token (GET)', () => {
        const result = { status: 'ok' };
        return request(app.getHttpServer())
            .get('/events-by-user/approve/token')
            .expect(200)
            .expect(({ body }) => {
