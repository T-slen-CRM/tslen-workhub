...
export class CallUsersOnlineComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  lastLang: string;
  columnDefs: any[];
  defaultColDef: any;
  rowSelection: string;
  ...
  ngOnInit(): void {
    ...
    const usersSub = this.dataService.getAgGridData('/users').subscribe(users => {
  ...
  loadColumnDefs(): void {
    this.translateService.get(['meet.call.list_name_company', 'meet.call.call_button']).subscribe(translations => {
      // this.columnDefs = [
      //   {
