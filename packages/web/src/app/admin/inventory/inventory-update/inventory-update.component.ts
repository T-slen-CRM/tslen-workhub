...
@Component({
    ...
    imports: [CommonModule, MatIconModule, ComponentsModule, DateTimeInputComponent, FormsModule, MatButtonModule, MatDialogModule, MatFormFieldModule, MatInputModule, MatOptionModule, MatSelectModule, ReactiveFormsModule, TranslateModule,],
    templateUrl: './inventory-update.component.html',
    styleUrl: './inventory-update.component.scss'
})
export class InventoryUpdateComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  ...
  ngOnInit(): void {
    ...
    saveInventory() {
      this.inventoryService.updateInventory(this.id, this.form.value, this.historyData, this.previousUserId).subscribe((response: any) => {
  ...
  changeDate(data: IInventoryHistory[]) {
    this.historyData = data.map(item => {
      item.formattedStartDate = this.inventoryService.formatDateReadable(item.startDate);
        item.formattedEndDate = this.inventoryService.formatDateReadable(item.endDate);
