...
describe('MeetingLinksController (e2e)', () => {
    ...
    it('never returns the raw token (or its hash) when listing the host\'s links', async () => {
        ...
        return request(app.getHttpServer())
            ...
            .expect(({ body }) => {
                ...
                // Assert on the whole serialised payload, not just the fields we
                // remembered to name - a future column added to the entity must not
                // be able to leak the secret through here unnoticed.
                const serialised = JSON.stringify(body);
                expect(serialised).not.toContain(validPlaintextToken);
