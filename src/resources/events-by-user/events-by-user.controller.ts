...
export class EventsByUserController {
  ...
  @Get('/pending')
  async getPending (@User() user: Users): Promise<EventsByUser[]> {
      return await this.eventsByUserService.getPending(user);
  }
  @SkipAuth()
  @Get('/approve/:token')
  async approveEvent (@Param('token') token: string): Promise<{status: string}> {
      return await this.eventsByUserService.approveDisapproveEvent(token, 1);
  }
  @SkipAuth()
  @Get('/disapprove/:token')
  async disapproveEvent (@Param('token') token: string): Promise<{status: string}> {
      return await this.eventsByUserService.approveDisapproveEvent(token, -1);
  }
}
