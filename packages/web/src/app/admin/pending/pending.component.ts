...
export class PendingComponent implements OnInit {
  ...
  loadTranslations(): void {
      this.translateService.get(['people.pending.name', 'people.pending.update_pending']).subscribe((translatedValue: string) => {
        this.mainHeaderPage = translatedValue['people.pending.name'];
        this.headerRoutes[0].value = translatedValue['people.pending.update_pending']
    });
  }
  ...
  changeUser(){
    this.dataService.changeUser({data: {userId: this.selectedUserId}}).subscribe(response => {
