...
} from "../../tslen-components/task-project-create-edit/task-project-create-edit.component";
...
import {IPermissionChangedRow, MatTableService} from "../../services/matTableService";
import { TranslateModule } from '@ngx-translate/core';

@Component({
    selector: 'app-tasks-manager',
    imports: [CommonModule, ReactiveFormsModule, MatCardModule,
        MatInputModule, MatIconModule, HelpersModule,
...
export class TasksManagerComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.taskProjects$ = this.dataService.getObservableData('/task-project')
        .pipe(map(r => {
    ...
    this.matTableService.changedMembersRow
        ...
        .subscribe((changedMembersRow: IPermissionChangedRow) => {
          ...
          if (changedMembersRow.action === 'delete'){
            ...
            const project = this.taskProjectDump.find((proj: ITaskProject) => proj.id === projectId);
            // remove permission by id
            project.taskProjectPermissions = project.taskProjectPermissions.filter((perm: IProjectPermission) => perm.id !== permissionId);
    ...
    if (!event.target.classList.contains('mat-icon')){
      this.router.navigate(['/pages/tasks-list/' + project.id, {title: project.name}]);
    }
    }
  createEditProject(type, project = null){
    ...
    const dialogRef = this.dialog.open(TaskProjectCreateEditComponent, dialogConfig );
  ...
  saveProject(project: ITaskProject){
    this.dataService.postData('/task-project', project)
        .pipe(takeUntil(this.destroy$))
  ...
  editProject(project: ITaskProject){
    this.taskProjects$ = this.dataService.updateData('/task-project/' , project.id, project)
        .pipe(
  ...
  saveProjectPermission(permissions: IProjectPermission[]){
    this.dataService.postData('/project-permission/create', {data: permissions})
        .pipe(takeUntil(this.destroy$))
        .subscribe((r: HttpResponse<any>) => {
          ...
          this.taskProjectDump = this.taskProjectDump.map((proj: ITaskProject) => {
            ...
            if (proj.id === projectPermission[0].projectId){
              proj.taskProjectPermissions = proj.taskProjectPermissions.concat(projectPermission);
            }
            return proj;
          });
          this.taskProjects$ = of(this.taskProjectDump);
  ...
  deleteProjectPermission(row: IProjectPermission){
    this.taskProjectDump = this.taskProjectDump.map((proj: ITaskProject) => {
      if (proj.id === row.projectId) {
        proj.taskProjectPermissions = proj.taskProjectPermissions
            .filter((perm: IProjectPermission) => perm.id !== row.id);
      }
      return proj;
    });
    this.taskProjects$ = of(this.taskProjectDump);
  ...
  async deleteProject(project: ITaskProject) {
    ...
    if (confirm) {
      this.taskProjects$ = this.dataService.deleteData('/task-project/', +project.id)
          .pipe(map(r => {
            this.taskProjectDump = this.taskProjectDump.filter((proj: ITaskProject) => proj.id !== project.id);
            return this.taskProjectDump;
          })
          );
    }
  ...
  openProjectMembers(project: ITaskProject){
    ...
    const dialogRef = this.dialog.open(TaskProjectMembersComponent, dialogConfig );
    dialogRef.afterClosed().subscribe(result => {
      if (result?.result){
        if (result.action === 'save'){
