...
export class UserGroupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const getUserGroup: Subscription = this.dataService.getObservableData('/groups')
        .subscribe(r => {
            if (Array.isArray(r)){
                this.rowData = r;
            } else {
  ...
  loadTranslations(): void {
    this.translateService.get([
          ...
          this.columnDefs = [
            {field: 'id', headerName: translations['people.user_group.data.id'], width: 386},
            {field: 'name', headerName: translations['people.user_group.data.name'], width: 387},
            {field: 'createdAt', headerName: translations['people.user_group.data.created'], cellRenderer: 'pendingDateRendererComponent', width: 387},
    ]})
  }
  onSubmit(){
      const save: Subscription = this.dataService.postData('/groups', this.form.value)
          .subscribe(r => {
              if (r.body){
