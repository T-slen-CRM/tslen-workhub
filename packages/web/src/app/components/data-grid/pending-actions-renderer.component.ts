...
export class PendingActionsRendererComponent implements OnDestroy{
    ...
    onDelete(){
        const dialogRef = this.dialog.open(DeleteConfirmModalComponent, {
            width: '400px',
            data: {text: 'Do you want to disapprove this event?', title: 'Disapproving'}
        });
        dialogRef.afterClosed().subscribe(result => {
            if (result){
    ...
    updateData(status){
        const updateData: Subscription = this.dataService.updateData('/events-by-user/',
            this.id,
            {id: this.id, approved: status, userId: this.userId})
            .subscribe((res) => {
