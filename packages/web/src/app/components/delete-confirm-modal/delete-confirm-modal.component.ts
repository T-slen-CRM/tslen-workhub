...
export class DeleteConfirmModalComponent implements OnInit {
  public title: string;
  public text: string;
  constructor(@Inject(MAT_DIALOG_DATA) public data: {text: string, title?: string}, public translateService: LanguageService) {
    this.title = this.data.title || 'Delete';
    this.text = this.data.text;
  }

  ngOnInit(): void {
    this.translateService.get(['task.phase.delete.text_delete', 'task.phase.delete.title_delete']).subscribe((transition)=>{
