...
import { CreateOneEventDialogComponent } from '../../../tslen-components/create-one-event-dialog/create-one-event-dialog.component';
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    const userData: Subscription = this.userData$.subscribe(
  ...
  openCreateOneEventDialog(
    ...
    const dialogRef = this.dialog.open(CreateOneEventDialogComponent, {
      ...
      data: {
        events,
        date,
        daysOffList: this.generalUserData.daysOff,
        isRequest,
      },
    });
