...
- `CHANGELOG.md` is auto-generated by [git-cliff](https://git-cliff.org/)
  (config: `cliff.toml`) — don't hand-edit it.
- To cut a release, push a tag matching `vX.Y.Z` (strict semver, no
  prerelease suffixes — the release workflow validates this and fails the
  tag's CI run otherwise) from `main`'s current tip. Pushing from
  anywhere else, or letting other commits land on `main` between tagging
  and the workflow running, can produce an incorrect changelog entry —
  always cut tags from `main`'s current tip.
- This triggers `.github/workflows/release.yml`, which regenerates
  `CHANGELOG.md`, commits it back to `main`, and publishes a GitHub
